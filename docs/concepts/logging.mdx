...
<Warning>
  Jangan pernah menulis token, cookie, OTP, challenge, QRIS asli, atau credential ke log. Library sudah menjaga ini: `HttpClient` mendeskripsikan request hanya sebagai host + path (bukan URL penuh yang bisa memuat query string sensitif), dan `HttpError` menyimpan body respons secara non-enumerable agar tidak tercetak `console.error`. Logger Anda tidak boleh mengembalikan disiplin ini. Lihat [Keamanan](/security/overview).
</Warning>
