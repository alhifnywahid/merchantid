...
<Note>
  Nama paket dan binary CLI adalah `merchantid`. Versi `0.1.0` sempat dipublikasikan dengan nama `merchid`; sejak `0.1.1` paket bernama `merchantid`. Jika Anda pernah memakai `merchid`, pasang ulang dengan `npm install merchantid` dan ganti perintah CLI dari `merchid` ke `merchantid`.
</Note>
